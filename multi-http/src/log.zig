...
const scope_levels = if (@hasDecl(root, "multi_http_scope_levels"))
    root.multi_http_scope_levels
else
    &[_]std.log.ScopeLevel{
        .{ .scope = .multi_connections, .level = .warn },
        .{ .scope = .multi_connections_thread, .level = .warn },
    };
